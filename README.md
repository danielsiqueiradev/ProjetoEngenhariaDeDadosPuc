...
A maior dificuldade inicial foi lidar com o trabalho pesado de *Data Wrangling* (concatenação de anos, correção de encoding UTF-8 e integração via API) e as limitações das fontes primárias, como a ausência de metadados regionais na rota padrão do TMDB. Além disso, tentamos estruturar um modelo preditivo de Machine Learning sobre o volume massivo de dados unificados, mas esbarramos em limitações de infraestrutura com *timeouts* recorrentes no cluster gratuito do Databricks. Diante disso, pivotamos com agilidade para focar nossos esforços na entrega de valor via Lakeview Dashboards, respondendo com precisão cirúrgica a todas as perguntas de negócio delineadas. 

Vale destacar o suporte colaborativo e o trabalho em conjunto com a inteligência artificial (Gemini) como parceira ativa de código e arquitetura ao longo de toda essa jornada de desenvolvimento. Como trabalho futuro, planeja-se otimizar a computação distribuída para reavaliar a camada de Machine Learning, automatizar a coleta 100% via *Web Scraping* ou integrar *endpoints* governamentais para cruzamento nativo e automatizado das classificações indicativas.
